...
% VERSION: v7 - Fixes critical bug in struct array handling from jsondecode.
    ...
    for i = 1:n
        ...
        % This now correctly returns a cell array of functions
...
function cell_val = get_cell_array_safely(struct_data, field_name)
    ...
    % VERSION 2: Correctly handles struct arrays produced by jsondecode.
    cell_val = {};
    if isfield(struct_data, field_name)
        temp_val = struct_data.(field_name);
        if iscell(temp_val)
            % It's already a cell array, we're good.
            cell_val = temp_val;
        elseif isstruct(temp_val)
            % --- THIS IS THE CRITICAL FIX ---
            if isscalar(temp_val)
                % It's a single JSON object (scalar struct), wrap it in a cell.
                cell_val = {temp_val};
            else
                % It's a JSON array of objects (non-scalar struct array).
                % Convert the struct array into a cell array of structs.
                cell_val = num2cell(temp_val);
            end
        end
    end
end
...
function truth_table = convert_to_truth_table_robust(func_data)
    ...
    if iscell(func_str), func_str = func_str{1}; end
    ...
    if num_inputs > 0
        truth_table = zeros(2^num_inputs, 1); % Ensure column vector
